...
float noise(float t){return texture2D(iChannel0,vec2(t,.0)/iChannelResolution[0].xy,0.0).x;}
float moy = 0.;
...
float noise(in vec3 p)
{
	...
	vec2 rz = texture2D( iChannel0, (tap+0.5)/256.0, 0.0 ).yx;
...
void main(void)
{
    ...
    mainImage(color, gl_FragCoord.xy + vec2(0.0*iViewpoint, 0.0));
    color.rgb = vec3(dot(vec3(0.3, 0.4, 0.3), color.rgb));
    color.w = 1.0;
    gl_FragColor = color;
}
